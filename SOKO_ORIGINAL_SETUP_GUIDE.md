# 🛒 Soko Platform — Complete Setup Guide
...
After unzipping `soko-platform.zip`, you'll have this structure:
...
# Create the database
psql -U postgres -c "CREATE DATABASE soko_db;"
psql -U postgres -c "CREATE USER soko_user WITH PASSWORD 'soko_pass';"
psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE soko_db TO soko_user;"
```

**Windows (after installing PostgreSQL):**
1. Open pgAdmin (installed with PostgreSQL)
2. Right-click "Databases" → Create → Database → name it `soko_db`
...
```env
DATABASE_URL=postgresql+asyncpg://soko_user:soko_pass@localhost:5432/soko_db
DATABASE_URL_SYNC=postgresql://soko_user:soko_pass@localhost:5432/soko_db
...
```env
DATABASE_URL=postgresql+asyncpg://postgres:postgres@localhost:5432/soko_db
...
DATABASE_URL_SYNC=postgresql://postgres:postgres@localhost:5432/soko_db
...
# In production: your registered AT username (e.g. "sokosupermarket")
...
# In sandbox: use "SOKO" or any name
...
3. Click **New project**
4. Choose a name (e.g. "soko-db"), set a **strong database password** (save it!), select **East Africa (closest region: Ohio or Frankfurt)**
...
4. Name it "soko-cache", select **Global** type, choose **EU-West** or **US-East**
5. Click **Create**
...
4. Name it "soko-production", give it **Full access**
...
2. Click **Add New App**
3. App Name: "Soko Supermarket" (or your name)
...
3. Set a **Secret Hash** (make one up, e.g. "soko-flw-webhook-2024")
...
# Check it's running
fly status
curl https://soko-api.fly.dev/health
```

Your API is now live at `https://soko-api.fly.dev`
...
# Project name: soko-frontend
# Directory: ./  (current)
# Override settings? N

# Set environment variables
vercel env add NEXT_PUBLIC_API_URL production
# Enter value: https://soko-api.fly.dev/api/v1

# Deploy to production
vercel --prod
```

Your storefront is now live at `https://soko-frontend.vercel.app`

### Step 3 — Update CORS

After deploying frontend, go back and update:
```bash
fly secrets set ALLOWED_ORIGINS="https://soko-frontend.vercel.app"
fly deploy
```
...
```bash
fly secrets set MPESA_CALLBACK_URL="https://soko-api.fly.dev/api/v1/payments/mpesa/callback"
fly deploy
```
...
```bash
curl -X POST https://soko-api.fly.dev/api/v1/auth/register \
...
TOKEN=$(curl -s -X POST https://soko-api.fly.dev/api/v1/auth/login \
  -H "Content-Type: application/json" \
...
Go to `https://soko-api.fly.dev/api/docs` → Categories → POST `/api/v1/categories`
...
*Soko Platform v1.0 — Phase 1 Complete*
